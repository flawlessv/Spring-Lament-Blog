...
在项目开发中，我们经常需要将页面内容导出为图片，特别是当导出内容与当前页面展示不同时。比如，导出报告时需要隐藏操作按钮、调整布局样式，或者使用不同的数据展示方式。

这个需求的核心挑战是：**html2canvas本身不支持差异化DOM导出，它只是一个截图工具，只能对当前可见的DOM进行截图。** 如果我们想要导出与页面展示不同的内容，就需要手动创建差异化的DOM结构。
...
第三，如果使用静态HTML字符串的方式，虽然性能好，但无法支持React的Hooks、Context等动态功能，适用场景非常有限。
...
默认情况下，html2canvas导出的图片可能比较模糊。我通过`scale`参数来提升清晰度，scale为2时，Canvas尺寸是实际尺寸的2倍，导出更清晰，但文件更大、处理更慢。我的策略是默认使用scale为2，在清晰度和性能之间取得平衡，同时提供配置选项，让用户可以根据需求调整。

**第四个难点：错误处理与用户体验**

导出过程可能因为各种原因失败，比如渲染失败、内存不足等。我添加了完善的错误处理：
...
第一，硬件加速。html2canvas内部使用Canvas API，浏览器会自动启用GPU加速，性能表现良好。

第二，按需渲染。只在需要导出时创建隐藏容器，导出完成后立即清理，避免长期占用内存。

第三，合理设置scale。根据实际需求设置scale值，平衡清晰度和性能。

### 最佳实践

第一，封装为自定义Hook。将导出逻辑封装为`useExportImage` Hook，方便复用和维护。

第二，支持配置化。导出尺寸、背景色、文件名等通过参数配置，提高灵活性。

第三，完善的错误处理。添加try-catch处理各种异常情况，提供用户友好的错误提示。

第四，资源清理。确保所有资源都能正确清理，避免内存泄漏。
...
**优化点一：只差异化一个 div 的导出样式（其余完全复用页面样式）**

假设要导出的区域有 5 个 div，只有 1 个 div 的导出样式和页面不同。这里我不会去维护“两套 DOM / 两套样式”，而是用“局部覆盖”的方式把差异压到最小：

- 我会给隐藏导出容器打一个作用域标记，比如 `data-export="1"`
- 给那个需要差异化的 div 打一个稳定标识，比如 `data-role="diff"`
- 然后只写一段导出态覆盖样式：`[data-export="1"] [data-role="diff"] { ... }`

这样导出时只有这一块产生样式差异，其余 4 个 div 完全沿用页面样式，不需要额外分支，也不会出现两个版本越改越不一致的问题。
