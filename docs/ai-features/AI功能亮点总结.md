...
本项目实现的 AI 功能属于 **LLM 应用开发（LLM Application Development）**，具体来说是：

- ✅ **RAG 应用开发**：实现了检索增强生成（RAG）模式
- ✅ **AI 功能集成**：将 LLM 能力集成到现有业务系统中
- ✅ **Prompt 工程**：通过精心设计的 Prompt 实现各种功能
- ❌ **不是 Agent 开发**：没有工具调用、自主决策、多步骤规划等 Agent 特性
- ❌ **不是 LangChain 开发**：没有使用 LangChain 框架，直接调用 LLM API

### 与 Agent 开发的区别

| 特性           | Agent 开发                           | 本项目                  |
| -------------- | ------------------------------------ | ----------------------- |
| **工具调用**   | ✅ 可以调用外部工具（API、数据库等） | ❌ 仅调用 LLM API       |
| **自主决策**   | ✅ 可以自主规划任务步骤              | ❌ 固定流程，无自主规划 |
| **反馈循环**   | ✅ 执行-评估-调整的循环              | ❌ 单次请求-响应        |
| **多步骤任务** | ✅ 可以分解复杂任务                  | ❌ 每个功能都是独立调用 |
| **状态管理**   | ✅ 维护任务状态和上下文              | ❌ 无状态，每次请求独立 |

### 与 LangChain 开发的区别

| 特性         | LangChain 开发               | 本项目               |
| ------------ | ---------------------------- | -------------------- |
| **框架**     | 使用 LangChain 框架          | 直接使用 OpenAI SDK  |
| **抽象层**   | Chains、Agents、Tools 等抽象 | 自定义函数封装       |
| **RAG 实现** | 使用 LangChain 的 RAG 链     | 自己实现向量检索逻辑 |
| **灵活性**   | 框架提供标准化流程           | 完全自定义实现       |
...
- **降级机制**：Ollama 不可用时自动降级为纯 LLM 模式，保证可用性
...
**问题**：用户快速输入时，多个 API 请求可能同时进行，后返回的请求可能覆盖新的补全建议。

**解决方案**：在防抖开始时记录光标位置，API 返回前后都校验位置，只有位置一致时才更新。

### 2. Ollama Embedding 的稳定性

**问题**：`nomic-embed-text` 对长文本不稳定，容易报错。
...
**问题**：外部 `value` 变化时，编辑器内容需要实时更新。

**解决方案**：

- 使用 `editorInstanceRef` 保存编辑器实例
- 通过 `editor.commands.setContent()` 更新内容
- 使用 `isUpdatingFromExternal` 标志防止循环更新

---

## 📈 性能优化

1. **防抖机制**：AI 补全使用 500ms 防抖，减少 API 调用
2. **上下文限制**：最多使用 500 字符作为上下文，避免 prompt 过长
3. **批量处理**：RAG 索引时批量处理，添加延迟避免过载
4. **缓存机制**：向量索引持久化，避免重复计算

---

## 🎯 总结

本博客系统的 AI 功能覆盖了内容创作的完整流程：
...
所有功能均采用**完全免费**的技术方案，在保证功能完整性的同时，实现了**隐私保护**和**用户体验**的平衡。
