# 启动指南 - Spring Broken AI Blog

本文档介绍如何启动 Spring Broken AI Blog 系统，包括基础服务和 AI 功能的配置。

## 📋 目录

- [快速启动](#快速启动) - 使用脚本快速启动
- [基础启动](#基础启动) - 仅启动博客系统
- [AI 功能启动](#ai-功能启动) - 启用 AI 功能的完整配置
- [服务检查](#服务检查) - 验证各服务运行状态
- [常见问题](#常见问题) - 问题排查

---

## 快速启动

项目提供了便捷的启动脚本来快速启动 AI 服务。

### 仅启动博客 (不含 AI)

```bash
# 1. 安装依赖
npm install

# 2. 配置环境变量
cp .env.example .env.local

# 3. 初始化数据库
npm run db:generate && npm run db:push && npm run db:seed

# 4. 启动开发服务器
npm run dev
```

### 完整启动 (含 AI 功能)

```bash
# 1. 安装依赖
npm install

# 2. 安装 Ollama (macOS)
brew install ollama

# 3. 启动 AI 服务
./start-ai.sh

# 4. 配置环境变量
cp .env.example .env.local
# 编辑 .env.local，添加 Kimi API Key

# 5. 初始化数据库
npm run db:generate && npm run db:push && npm run db:seed

# 6. 启动开发服务器
npm run dev
```

**停止 AI 服务:**

```bash
./stop-ai.sh
```

---

## 基础启动

如果你只需要博客的基础功能 (不使用 AI 功能)，只需执行以下步骤:

### 1. 安装依赖

```bash
npm install
```

### 2. 配置环境变量

创建 `.env.local` 文件:

```bash
# 数据库配置
DATABASE_URL="file:./prisma/dev.db"

# NextAuth 配置
NEXTAUTH_SECRET="your-secret-key-at-least-32-characters-long"
NEXTAUTH_URL="http://localhost:7777"

# 管理员账户 (seed 时使用)
ADMIN_USERNAME="admin"
ADMIN_PASSWORD="0919"
```

### 3. 初始化数据库

```bash
# 生成 Prisma 客户端
npm run db:generate

# 推送数据库架构
npm run db:push

# 填充初始数据
npm run db:seed
```

### 4. 启动开发服务器

```bash
npm run dev
```

访问 http://localhost:7777 即可使用博客的基础功能。

---

## AI 功能启动

如需使用 AI 功能 (智能写作助手、RAG 聊天等)，需要额外安装和配置以下服务:

### 方式一: 使用启动脚本 (推荐)

项目提供了 `start-ai.sh` 和 `stop-ai.sh` 脚本来管理 AI 服务。

```bash
# 启动 AI 服务
./start-ai.sh

# 停止 AI 服务
./stop-ai.sh
```

**启动脚本会自动完成:**

- 启动 Ollama 服务
- 启动 ChromaDB 服务
- 下载 nomic-embed-text 模型 (首次运行)
- 验证服务运行状态

### 方式二: 手动启动

如果你想手动启动各个服务，请按以下步骤操作:

#### 1. 安装 Ollama (用于向量生成)

**macOS:**

```bash
brew install ollama
```

**Linux:**

```bash
curl -fsSL https://ollama.com/install.sh | sh
```

**启动 Ollama:**

```bash
ollama serve
```

#### 2. 安装并启动 ChromaDB (用于向量存储)

**使用 npx (推荐):**

```bash
npx chromadb run --path ./data/chroma --port 8000
```

**或使用 Docker:**

```bash
docker run -d --name chromadb -p 8000:8000 chromadb/chroma:latest
```

**或使用 Python:**

```bash
pip install chromadb
chroma run --host localhost --port 8000
```

#### 3. 配置 AI 环境变量

在 `.env.local` 文件中添加:

```bash
# AI 配置
KIMI_API_KEY="sk-your-kimi-api-key"
KIMI_BASE_URL="https://api.moonshot.cn/v1"
KIMI_MODEL="moonshot-v1-32k"

# Ollama 配置
OLLAMA_BASE_URL="http://localhost:11434"
OLLAMA_EMBEDDING_MODEL="nomic-embed-text"

# ChromaDB 配置
CHROMADB_HOST="localhost"
CHROMADB_PORT="8000"
```

**获取 Kimi API Key:**

1. 访问 https://platform.moonshot.cn/
2. 注册/登录账号
3. 进入 API Keys 页面创建密钥

---

## 服务检查

启动完成后，验证各服务运行状态:

### 检查 Ollama

```bash
curl http://localhost:11434/api/tags
```

**预期输出:**
包含 `nomic-embed-text` 模型的列表

### 检查 ChromaDB

```bash
curl http://localhost:8000/api/v1/heartbeat
```

**预期输出:**

```json
{ "status": "ok" }
```

### 检查博客服务

访问 http://localhost:7777，检查:

- ✅ 首页正常加载
- ✅ 可以登录后台 (`/login` 或 `/admin`)
- ✅ AI 功能页面可访问

---

## 常见问题

### Ollama 连接失败

**错误:** `Failed to connect to localhost:11434`

**解决方案:**

```bash
# 检查是否运行
ps aux | grep ollama

# 重启 Ollama
pkill ollama && ollama serve

# 确认模型已下载
ollama list
```

### ChromaDB 连接失败

**错误:** `ECONNREFUSED localhost:8000`

**解决方案:**

```bash
# Docker 方式
docker ps | grep chromadb
docker restart chromadb

# npx 方式
pkill -f "chroma run"
npx chromadb run --path ./data/chroma --port 8000
```

### Kimi API 调用失败

**错误:** `请配置 KIMI_API_KEY`

**解决方案:**

1. 检查 `.env.local` 是否包含 `KIMI_API_KEY`
2. 验证 API Key 是否有效
3. 重启开发服务器: `npm run dev`

### 向量索引失败

**解决方案:**

1. 确保 Ollama 和 ChromaDB 都在运行
2. 测试 Ollama Embedding:
   ```bash
   curl http://localhost:11434/api/embeddings \
     -d '{"model": "nomic-embed-text", "prompt": "测试"}'
   ```
3. 查看浏览器控制台错误日志

### AI 功能不显示

**解决方案:**

1. 确认已配置所有 AI 环境变量
2. 确认 AI 服务正在运行
3. 重启开发服务器

---

## 下一步

启动成功后:

1. **访问博客:** http://localhost:7777
2. **登录后台:** http://localhost:7777/login
   - 用户名: `admin`
   - 密码: `0919`
3. **测试 AI 功能:**
   - 进入后台管理
   - 打开 AI 写作助手
   - 测试 RAG 聊天功能

**提示:** 首次使用 AI 功能时，需要先发布几篇文章并进行向量索引，才能使用 RAG 聊天功能。

---

## 相关文档

- [README.md](../README.md) - 项目概览
- [CLAUDE.md](../CLAUDE.md) - 开发指南
- [部署指南](./部署指南.md) - 生产环境部署
- [AI功能亮点总结](./ai-features/AI功能亮点总结.md) - AI 功能详解
